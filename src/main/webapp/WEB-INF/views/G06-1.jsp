...
<body>
   ...
   <!-- 화면에서 가동시간을 계산해서 동적으로 화면에 나타내기 위한 부분 -->
			...
			<table id="monthtable" class="scrolltable">
               ...
               op_time['<c:out  value="${x}"/>'] = '<c:out value="${w_time}"/>';<!-- 각 날짜별 가동시간 저장 -->
					...
					<tr>
						...
						</select> <select id="in_minute<c:out value = "${x}"/>" name="in_minute"
						...
						<td><select id="out_hour<c:out value = "${x}"/>"
							name="out_hour" onchange="setouthour(<c:out value = "${x}"/>)">
								<!-- 퇴근 시각 출력 -->
								<c:forEach var="i" begin="0" end="23" step="1">
									<c:choose>
						...
						</select> <select id="out_minute<c:out value = "${x}"/>" name="out_minute"
						...
						<td>
							<div id="time<c:out value = "${x}"/>">
								<!-- 가동시간 출력 -->
								<input id="op_time<c:out value = "${x}"/>" name="op_time"
									type="hidden" value="${w_time }" />
								<!-- 동적으로 변한값  -->
								<c:out value="${w_time}" />
								<!-- 기본 출력값  -->
							</div>
						</td>
						<td><select id="w_list<c:out value = "${x}"/>" name="w_list"
							class="select font">
								<!-- wco_list 출력  -->
								<option>=休憩=</option>
						...
						<td><select id="br_time<c:out value = "${x}"/>"
							name="br_time" onchange="setfreetime(<c:out value = "${x}"/>)">
								<!-- 휴식시간 출력  -->
						...
						<td><input id="notice<c:out value = "${x}"/>" name="notice"
							type="text" maxlength="200" style="width: 300px;"
							value="${list.notice}" /></td>
						<!-- 작업내역 출럭  -->
						<td><input style="width: 400px;"
							id="remark<c:out value = "${x}"/>" name="remark" type="text"
							maxlength="200" value="${list.remarks}" /></td>
						<!-- 비고 출력  -->
					</tr>
					<c:set var="x" value="${x+1 }" />
				</c:forEach>
				<tr>
					<td><input id="op_time" name="op_time" type="hidden"
						value="${w_time }" /> <!-- 가동시간의 히든값(버그대응용)  --></td>
				</tr>
			</table>
			<input type="hidden" id="month" name="month" value="<%=month%>" />
			<!-- 다음페이지에 현제 달을 넘기기위한 히든값  -->
			<input type="hidden" id="year" name="year" value="<%=year%>" />
			<!-- 다음페이지에 현제 년을 넘기기위한 히든값  -->
			<input type="submit" style="width: 70px; float: right" value="保存" />
			<!-- 일반 보존버튼. attendanceUpdate로 넘어간다 -->
			<input type="submit" style="width: 150px; float: right"
				onclick="btn_click('saveWithExcel')" value="保存後EXCEL出力">
			<!-- 보존 및 엑셀출력 버튼 엑셀쪽으로 넘어간다 -->
	...
	<form action="mainPage" method="get">
		<input type="submit" style="width: 100px" value="キャンセル">
		<!-- 전 페이지로 되돌아가기  -->
	</form>
	<!-- 追加 -->
